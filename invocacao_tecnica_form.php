...
$data   = ['nome' => '', 'descricao' => ''];
if ($id) {
    $s = $conn->prepare("SELECT nome, descricao FROM invocacao_tecnicas WHERE id=? AND id_invocacao=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        $s = $conn->prepare("UPDATE invocacao_tecnicas SET nome=?,descricao=? WHERE id=? AND id_invocacao=?");
        $s->bind_param("ssii", $nome, $desc, $id, $inv_id);
    } else {
        $s = $conn->prepare("INSERT INTO invocacao_tecnicas (id_invocacao,nome,descricao) VALUES (?,?,?)");
