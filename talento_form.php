...
$data = ['nome'=>'','descricao'=>''];
if ($id) {
    $s = $conn->prepare("SELECT nome, descricao FROM player_talentos WHERE id=? AND id_player=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        $s = $conn->prepare("UPDATE player_talentos SET nome=?,descricao=? WHERE id=? AND id_player=?");
        $s->bind_param("ssii",$nome,$desc,$id,$pid);
    } else {
        $s = $conn->prepare("INSERT INTO player_talentos (id_player,nome,descricao) VALUES (?,?,?)");
