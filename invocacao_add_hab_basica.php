...
$habs = $conn->query("SELECT id, nome FROM habilidades_basicas ORDER BY nome")->fetch_all(MYSQLI_ASSOC);
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $hab_id = intval($_POST['id_habilidade']);
    $level  = intval($_POST['level']);
    $s = $conn->prepare("INSERT INTO R_invocacao_habilidades_basicas (id_invocacao, id_habilidade_basica, level) VALUES (?,?,?)");
...
<div class="content"><form class="admin-form" method="post">
    <h2>Adicionar Habilidade Básica</h2>
